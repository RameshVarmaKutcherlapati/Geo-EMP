select GEOROWID                   as geoRowID,
       ...
       POSTALCODEMANDATORY        as postalCodeMandatory,
...
from (
select A.rowid_object        as GeoRowID,
             O.CODE as GeoId,
      ...
      from MDM_INFM_SMDS.C_GDA_DFND_AREA A
               ...
               LEFT OUTER JOIN (select B.CODE,B.GDA_DFND_AREA_ROWID from C_ALT_CODE B
                                INNER JOIN C_TYP_TYPE C ON C.ROWID_OBJECT=B.TYP_TYPE_ROWID AND C.NAME='GEOID' AND C.HUB_STATE_IND=1
                                WHERE B.HUB_STATE_IND=1) O ON O.GDA_DFND_AREA_ROWID= A.ROWID_OBJECT
